      ...
      <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
        <div class="container mx-auto px-6 py-8">
          <h3
            ...
            Edit Invoice : {{ $invoice->invoice_number }}</h3>
              ...
              <div class="flex justify-center items-center w-full">
                <div class="w-1/2 bg-gray-700 sm:rounded-lg shadow-2xl p-8 m-4">
                  ...
                  <form action="/editinvoice/update/{{ $invoice->id }}" method="post">
                    ...
                    <div class="flex flex-col mb-4">
                      <label class="mb-2 font-bold text-lg text-white" for="updated_at">Invoice Date</label>
                      ...
                      <select name="company_id" id="company_id" class="border py-2 px-3 text-grey-80 ">
                        ...
                        @foreach ($companies as $company)
                          <option value="{{ $company->id }}"
                            {{ $company->id === $invoice->contact->company->id ? 'selected' : '' }}>
                            {{ $company->name }}
                          </option>
                        @endforeach
                      </select>
